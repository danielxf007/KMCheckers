...
func set_game_pices(type: int, player_id: int, n_rows: int,
row_offset: int, init_col_pos: int) -> void:
	var col_pos: int = init_col_pos
	for i in range(n_rows):
		for j in range(col_pos, int(self.BOARD_DIMENSIONS.x), 2):
			self.game_board[i+row_offset][j].set_type(type)
			self.game_board[i+row_offset][j].set_player_id(player_id)
...
func _on_GameSetUpUI_game_started(game_setup_data: Dictionary) -> void:
	...
	self.set_game_pices(player_1_piece_type, PLAYER_1_ID, 3, 0, 1)
	var player_2_piece_type: int = (player_1_piece_type+1)%2
	self.set_game_pices(player_2_piece_type, PLAYER_2_ID, 3, 5, 0)
